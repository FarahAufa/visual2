...
uses
  FMX.Controls, Classes, Variants,
  SysUtils, FMX.Forms, FMX.Menus, FMX.Types, FMX.Edit, System.UITypes, Types, FMX.Platform, FMX.TreeView, FMX.Layouts;
...
type
  ...
  TfsSyntaxMemo = class(TStyledControl)
  private
  ...
  protected
    ...
    procedure DOver(Sender: TObject; const Data: TDragObject; const Point: TPointF; var Accept: Boolean);
...
begin
  ...
  begin
    ...
    MeasureText(r, 'W', False, [], TTextAlign.taLeading);
...
begin
  if (not FReadOnly) then
    SelText := VarToStr(Platform.GetClipboard);
end;
...
begin
  ...
  begin
    if(PopupMenu = nil) then FPopUpMenu.Popup(Platform.GetMousePos.X, Platform.GetMousePos.Y);
  end
  else
  begin
    FMoved := True;
...
procedure TfsSyntaxMemo.DOver(Sender: TObject; const Data: TDragObject; const Point: TPointF; var Accept: Boolean);
begin
