...
, FMX.Types, FMX.Objects, System.UITypes, System.UIConsts, Types, Variants;
...
begin
  ...
  begin
    ...
    begin
      ...
      AddMethod('procedure DrawThumbnail(ABitmap: TBitmap; Width: Single; Height: Single)', CallMethod);
    ...
    begin
      ...
      AddMethod('procedure FillColor(Color: TAlphaColor)', CallMethod);
      AddMethod('procedure LoadFromFile(AFileName: string; Rotate: Single = 0)', CallMethod);
      AddMethod('procedure SaveToFile(AFileName: string; Params: string = '')', CallMethod);
...
begin
  ...
  begin
    ...
    begin
      ...
      _Bitmap.InvertAlpha
    else if MethodName = 'FILLCOLOR' then
      _Bitmap.FillColor(TAlphaColor(Caller.Params[0]))
      ...
      _Bitmap.SaveToFile(String(Caller.Params[0]), String(Caller.Params[1]))
  ...
  begin
    ...
    if MethodName = 'CREATE' then
      Result := frxInteger(TCanvas(DefaultCanvasClass).Create)
    else if MethodName = 'BEGINSCENE' then
   ...
   { else if MethodName = 'SETCLIPRECTS' then
    ...
    else if MethodName = 'DRAWTHUMBNAIL' then
      _Canvas.DrawThumbnail(TBitmap(frxInteger(Caller.Params[0])), Single(Caller.Params[1]), Single(Caller.Params[2]))
