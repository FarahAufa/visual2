...
uses
  SysUtils, Classes, FMX.fs_iinterpreter, FMX.fs_ievents, FMX.fs_iclassesrtti,
...
{$ENDIF}
    ...
    begin
      ...
      AddMethod('procedure ShowCaretProc()', CallMethod);
      AddMethod('procedure SetCaretPos(APoint: TfsPointF)', CallMethod);
      AddMethod('procedure SetCaretSize(ASize: TfsPointF)', CallMethod);
      AddMethod('procedure SetCaretColor(AColor: TAlphaColor)', CallMethod);
      AddMethod('procedure HideCaret()', CallMethod);
      AddMethod('procedure BeginUpdate()', CallMethod);
      AddMethod('procedure EndUpdate()', CallMethod);
      AddMethod('procedure Realign()', CallMethod);
      ...
      AddProperty('DisableDefaultAlign', 'Boolean', GetProp, SetProp);
    ...
    begin
      ...
      AddProperty('BindingSource', 'TStyledControl', GetProp, SetProp);
      AddProperty('AutoTranslate', 'Boolean', GetProp, SetProp);
      AddProperty('Action', 'TBasicAction', GetProp, SetProp);
    end;
    ...
    begin
      ...
      AddProperty('ShowCaret', 'Boolean', GetProp, SetProp);
      AddProperty('FontFill', 'TBrush', GetProp, nil);
...
begin
  ...
  begin
    ...
    else if MethodName = 'SHOWCARETPROC' then
      TControl(Instance).ShowCaretProc()
    else if MethodName = 'SETCARETPOS' then
      TControl(Instance).SetCaretPos(TfsPointF(frxInteger(Caller.Params[0])).GetRect)
    else if MethodName = 'SETCARETSIZE' then
      TControl(Instance).SetCaretSize(TfsPointF(frxInteger(Caller.Params[0])).GetRect)
    else if MethodName = 'SETCARETCOLOR' then
      TControl(Instance).SetCaretColor(TAlphaColor(Caller.Params[0]))
    else if MethodName = 'HIDECARET' then
      TControl(Instance).HideCaret()
    ...
    else if MethodName = 'REALIGN' then
      TControl(Instance).Realign()
  ...
  begin
    ...
    else if MethodName = 'COPYTOCLIPBOARD' then
...
begin
  ...
  begin
    ...
    else if PropName = 'DISABLEDEFAULTALIGN' then
      Result := TControl(Instance).DisableDefaultAlign
  ...
  begin
    if PropName = 'BINDINGSOURCE' then
      Result := frxInteger(TStyledControl(Instance).BindingSource)
    else if PropName = 'AUTOTRANSLATE' then
      Result := TStyledControl(Instance).AutoTranslate
    else if PropName = 'ACTION' then
      Result := frxInteger(TStyledControl(Instance).Action)
  end
  else if ClassType = TCustomEdit then
  begin
    ...
    else if PropName = 'SHOWCARET' then
      Result := TCustomEdit(Instance).ShowCaret
    else if PropName = 'FONTFILL' then
      Result := frxInteger(TCustomEdit(Instance).FontFill)
  ...
  begin
    if PropName = 'SELSTART' then
      ...
      Result := frxInteger(TMemo(Instance).FontFill)
    else if PropName = 'SELECTIONFILL' then
      Result := frxInteger(TMemo(Instance).SelectionFill)
  end
...
begin
  ...
  begin
    if PropName = 'AUTOCAPTURE' then
      ...
      TControl(Instance).DisableDefaultAlign := Boolean(Value)
    else if PropName = 'TABORDER' then
  ...
  begin
    if PropName = 'BINDINGSOURCE' then
      TStyledControl(Instance).BindingSource := TStyledControl(frxInteger(Value))
    else if PropName = 'AUTOTRANSLATE' then
      TStyledControl(Instance).AutoTranslate := Value
    else if PropName = 'ACTION' then
      TStyledControl(Instance).Action := TBasicAction(frxInteger(Value))
  end
  else if ClassType = TCustomEdit then
  begin
    if PropName = 'CARETPOSITION' then
      ...
      TCustomEdit(Instance).ShowCaret := Boolean(Value)
    else if PropName = 'FILTERCHAR' then
